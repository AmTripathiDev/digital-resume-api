...
export class UserController {
  ...
  // })
  // .catch((err) => {
  //   console.log(err, " NodeMailer Error ");

  static verify(req: Request, res: Response) {
    const code = req.params.code;
    User.findOneAndUpdate({ code: code }, { verified: true })
      .exec()
      .then((user) => {
        if (user) {
          ...
          res.send(
            "email not verified. please try by clicking send email again"
          );
        }
      })
      .catch((err) => {
  ...
  static getUserDetails(req: Request, res: Response) {
    ...
    User.findOneAndUpdate({ _id: userId }, { last_active: date }, { new: true })
      .select(
        "onboarding verified _id email password name code last_active experience_level job_category"
      )
      .then((data) => {
  ...
  static updateName(req: Request, res: Response) {
    ...
    if (!name) {
      ...
      User.findOneAndUpdate({ _id: userId }, { name: name }, { new: true })
        .exec()
        .then((data) => {
          res.status(200).send(data);
        })
    ...
    if (!email) {
      ...
      User.findOne({ email: email }).then((data) => {
        ...
        Nodemailer.sendEmail(
          " Digi Resume <contact@digiresumes.com>",
          email,
